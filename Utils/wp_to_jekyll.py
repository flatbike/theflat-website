#!/usr/bin/env python3
"""
wp_to_jekyll.py — Convert a WordPress WXR export to Jekyll markdown files.
...
# ── Illegal XML character sanitizer ─────────────────────────────────────────
...
def sanitize_xml(xml_file):
    ...
    return _ILLEGAL_XML_CHARS.sub('', text).encode('utf-8')
...
def extract_posts(xml_file, include_drafts=False):
    ...
    clean_bytes = sanitize_xml(xml_file)

    try:
        root = ET.fromstring(clean_bytes)
        channel = root.find('channel')
    except ET.ParseError as e:
        print(f"⚠️  Standard parser failed ({e}), trying lxml recovery mode...")
        try:
            from lxml import etree as lxml_et
        except ImportError:
            raise RuntimeError(
                "lxml is needed for recovery mode. Install it with:\n"
                "  pip install lxml"
            ) from e
        parser = lxml_et.XMLParser(recover=True, encoding='utf-8')
        lxml_root = lxml_et.fromstring(clean_bytes, parser)
        root = ET.fromstring(lxml_et.tostring(lxml_root))
        channel = root.find('channel')

    if channel is None:
        raise ValueError("No <channel> found — is this a valid WXR export file?")
...
def main():
    ...
    for post in posts:
        ...
        print(f"  ✓  {fname}  —  {post['title'][:55]}{draft_tag}")

    print(f"\n🎉  {len(posts)} file(s) written to '{out}/'")

    print("""
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  JEKYLL SETUP — HOW TO USE YOUR ARCHIVE
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

OPTION A  ──  Drop into _posts/ (simplest, one line)
  Just move the .md files into your Jekyll site's _posts/
  folder. They'll be treated as regular posts.

OPTION B  ──  Separate collection (cleaner for archives)
